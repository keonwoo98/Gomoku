...
### 5.4 Dynamic Heuristic (게임 페이즈 인식)

평가 함수는 게임 진행 단계에 따라 가중치를 동적으로 조정합니다:

```rust
enum GamePhase {
    Opening,   // 0-10 total stones+captures
    Midgame,   // 11-40
    Endgame,   // 41+
}
```
...
**페이즈별 가중치 조정:**
| 페이즈 | 위치 가중치 | 취약성 가중치 | 캡처 가중치 |
|--------|-----------|-------------|-----------|
| Opening | 1.5x | 0.5x | 0.8x |
| Midgame | 1.0x (기준) | 1.0x | 1.0x |
| Endgame | 0.6x | 1.5x | 1.3x |

- **Opening**: 중앙 위치 선점이 중요, 캡처 위험 낮음
- **Midgame**: 균형잡힌 평가
- **Endgame**: 캡처 위협과 취약성이 결정적, 위치 중요도 감소

Negamax 대칭성 보장: `detect_phase(board)`는 color 무관, 모든 가중치가 양쪽 동일하게 적용.
...
### Opening Rules (Starting Conditions)

게임 시작 시 세 가지 오프닝 룰을 선택할 수 있습니다:

- **Standard**: 제한 없이 자유롭게 플레이
- **Pro**: 첫 수는 중앙(K10), 세 번째 수는 중앙에서 3칸 이상 떨어진 곳
- **Swap**: 3수 후 후수(White) 플레이어가 색상 교환 선택 가능

### AI vs AI 관전 모드

`GameMode::AiVsAi` — 두 AI가 대전하는 것을 관전하는 모드. 디버그 패널에서 양측의 탐색 통계를 실시간으로 확인할 수 있습니다. 오프닝 룰(Standard/Pro/Swap) 선택 가능.
...
└── ui/
    ├── mod.rs              # UI 모듈 정의
    ├── app.rs              # Main application and side panel rendering
    ├── board_view.rs       # Board rendering and mouse interaction
